...
BAYESIAN_STATIC_WEEK1_PRIOR = True
...
if __name__ == "__main__":
    ...
    # 统计：完全没命中（没有任何有效样本）的周数
    # 说明：当 run_simulation_percent_mode 返回 result_df=None 时，表示 valid_count==0
    miss_stats = {
        'overall': {
            'weeks_attempted': 0,
            'weeks_no_hit': 0,
    ...
    for prior_label, use_bayesian in prior_modes:
        if prior_label not in miss_stats:
            miss_stats[prior_label] = {
                'weeks_attempted': 0,
                'weeks_no_hit': 0,
                'elim_weeks_attempted': 0,
                'elim_weeks_no_hit': 0,
        ...
        for season in TARGET_SEASONS:
            ...
            for week in weeks_to_simulate:
                ...
                # 统计“完全没命中”周：result_df 为 None 表示该周没有任何有效样本
                miss_stats['overall']['weeks_attempted'] += 1
                miss_stats[prior_label]['weeks_attempted'] += 1
                if had_elimination:
                    miss_stats['overall']['elim_weeks_attempted'] += 1
                    miss_stats[prior_label]['elim_weeks_attempted'] += 1

                if result_df is None:
                    miss_stats['overall']['weeks_no_hit'] += 1
                    miss_stats[prior_label]['weeks_no_hit'] += 1
                    if had_elimination:
                        miss_stats['overall']['elim_weeks_no_hit'] += 1
                        miss_stats[prior_label]['elim_weeks_no_hit'] += 1
                ...
                if result_df is not None:
                    ...
                    if had_elimination:
                        if use_bayesian and BAYESIAN_STATIC_WEEK1_PRIOR:
                            # 静态第1周先验：不更新
                            pass
                        else:
    ...
    # ---------------------------------------------------------
    print("\n" + "=" * 70)
    print("完全没命中周数统计（Valid 样本数=0）")
    print("=" * 70)
    for k in ['bayesian', 'uniform']:
        if k in miss_stats:
            s = miss_stats[k]
            print(
                f"{k.upper():<8} | No-hit weeks: {s['weeks_no_hit']}/{s['weeks_attempted']}"
                f" | No-hit elimination weeks: {s['elim_weeks_no_hit']}/{s['elim_weeks_attempted']}"
            )
    s_all = miss_stats['overall']
    print(
        f"OVERALL  | No-hit weeks: {s_all['weeks_no_hit']}/{s_all['weeks_attempted']}"
        f" | No-hit elimination weeks: {s_all['elim_weeks_no_hit']}/{s_all['elim_weeks_attempted']}"
    )
